...
export class CollapsibleContainerNode extends ElementNode {
  ...
  getOpen() {
    // FIX: avoid possible fail if node is not in lexical state
    try {
  ...
  toggleOpen() {
    // FIX - only if bool is returned
    let latestOpen = this.getOpen();
    if (latestOpen === null) return;
    this.setOpen(!latestOpen);
  }
}
